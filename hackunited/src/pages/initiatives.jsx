...
function InitiativeCard({ title, icon, items }) {
  return (
    ...
    >
      <Card className="bg-gradient-to-br from-black/60 to-black/30 backdrop-blur-lg border border-purple-800 shadow-xl rounded-3xl w-full sm:max-w-xs p-0">
        <CardContent className="p-6 space-y-4">
          <div className="flex items-center gap-3">
            {icon}
            <h2 className="text-xl font-semibold bg-clip-text text-transparent bg-gradient-to-r from-purple-300 to-purple-500">
              {title}
            </h2>
          </div>
          <Separator className="bg-purple-700 w-1/2" />
          <ul className="space-y-2 text-sm leading-relaxed text-purple-100">
